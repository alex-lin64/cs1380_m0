...
if $DIFF <(sort d/visited.txt) <(sort "$T_FOLDER"/d/s2_urls.txt) > /dev/null;
then
...
if DIFF_PERCENT=$DIFF_PERCENT t/gi-diff.js <(sort d/global-index.txt) <(sort "$T_FOLDER"/d/s2_gi.txt) > /dev/null;
then
